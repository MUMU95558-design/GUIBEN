...
export default function ActionSuggestion({ action, onAccept, onDecline }: ActionSuggestionProps) {
  return (
    <div className="bg-gradient-to-br from-orange-50 to-indigo-50 border border-orange-100/50 rounded-2xl p-6 mb-6 shadow-sm">
      <div className="flex items-start gap-3 mb-3">
        <div className="flex-shrink-0 w-8 h-8 bg-[#FF8A65] rounded-lg flex items-center justify-center">
      ...
      <div className="flex gap-3">
        <button
          onClick={onAccept}
          className="flex-1 bg-[#FF8A65] text-white text-sm py-3 rounded-full hover:shadow-lg hover:shadow-orange-400/30 transition-all duration-300 font-medium hover:scale-105"
        >
          去做
        </button>
        <button
